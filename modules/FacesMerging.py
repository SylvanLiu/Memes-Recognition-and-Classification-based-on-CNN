...
def new_merge(points_Set, images_Matrix, w=600):

    # Read points for all images
    allPoints = points_Set
    # Read all images
    images = images_Matrix
    ...
    pointsAvg = np.float32(
        np.array([(0, 0)] * (len(allPoints[0]) + len(boundaryPts))))
    n = len(allPoints[0])
    ...
    # Output image
    output = np.float32(np.zeros((h, w, 3)))
    # Warp input images to average image landmarks
    for i in range(0, len(imagesNorm)):
        img = np.float32(np.zeros((h, w, 3)))
    ...
    # Adjust BGR Seq to RGB.
    # print('\n' + 'adjusted_output: ' + '\n'+str(adjusted_output)+'\n')
    return output


def mergingFaces(path):
    # Read points for all images
    ...
    # print('\n' + 'allPoints: ' + '\n'+str(allPoints)+'\n')
    # print('\n' + 'images: ' + '\n'+str(images)+'\n')
    new_merge(allPoints, images)


if __name__ == '__main__':
    # Print launch time for this program.
    start_Time = time.time()
    mergingFaces('/Users/liusiyuan/Desktop/Codes/Img4test/presidents_Test')
    end_Time = time.time()
